...
public class SolrRecordIndex implements RecordIndex{
    ...
    private static SolrClient baseSolr = null;
    ...
    private static List<SolrClient> shSolr = null;
    ...
    public void connect() throws MalformedURLException{
        ...
        if( shardUrl.size() == 0 && baseSolr == null ){
            ...
            baseSolr = new ConcurrentUpdateSolrClient.Builder( baseUrl ).build();
        ...
        if( shardUrl.size() > 0 && shSolr == null ){
            ...
            shSolr = new ArrayList<SolrClient>();
            ...
            for( Iterator<String> is = shardUrl.iterator(); is.hasNext(); ){
                ...
                SolrClient cuss = new ConcurrentUpdateSolrClient.Builder( is.next() ).build();
    ...
    public void commit()
        ...
        if( shSolr != null && shSolr.size() > 0 ){
            ...
            for( Iterator<SolrClient> is = shSolr.iterator(); is.hasNext(); ){
                is.next().commit();
            } 
        } else {
            baseSolr.commit();
    ...
    public void clear(){
        ...
        try{
            if( shSolr != null && shSolr.size()>0 ){
                ...
                for( Iterator<SolrClient> is = shSolr.iterator(); 
                     is.hasNext(); ){
                    
                    //SolrServer css = is.next();
                    SolrClient css = is.next();
                    css.deleteByQuery( "*:*" );
                    css.commit();
    ...
    public Map getMeta(){
        ...
        if( baseUrl != null ){
            ...
            System.out.println( "getMeta: baseUrl=" + baseUrl );
